...
func InformacionCertificacionDve(numeroDocumento []string, periodoInicial []string, periodoFinal []string, vinculaciones []string, incluirSalario bool) (certificacion models.InformacionCertificacionDve, outputError map[string]interface{}) {
	...
	docente.NivelAcademico = intensidadDocente[0].Resoluciones[0].NivelAcademico
	docente.Facultad = intensidadDocente[0].Resoluciones[0].Facultad
	docente.ProyectoCurricular = intensidadDocente[0].Resoluciones[0].ProyectoCurricular
	docente.Categoria = intensidadDocente[0].Resoluciones[0].Categoria
	docente.Dedicacion = intensidadDocente[0].Resoluciones[0].TipoVinculacion

	certificacion = models.InformacionCertificacionDve{
		InformacionDve:    docente,
		IntensidadHoraria: intensidadDocente,
	}
...
func IntensidadHorariaDve(numeroDocumento []string, periodoInicial []string, periodoFinal []string, vinculaciones []string) (intensidadDocente []models.IntensidadesDocente, ultimaVinculacion models.Certificado, outputError map[string]interface{}) {
	...
	for _, intensidad := range intensidadHoraria {
		if anioActual != intensidad.Anio || periodoActual != intensidad.Periodo {
			anioActual = intensidad.Anio
			periodoActual = intensidad.Periodo

			var intensidadIndividual models.IntensidadesDocente
			intensidadIndividual.Anio = anioActual
			intensidadIndividual.Periodo = periodoActual
			intensidadIndividual.TipoVinculacion = intensidad.TipoVinculacion

			intensidadDocente = append(intensidadDocente, intensidadIndividual)
		}

		for i := range intensidadDocente {
			if intensidadDocente[i].Anio == anioActual && intensidadDocente[i].Periodo == periodoActual {
				intensidadDocente[i].NumeroSemanas += intensidad.NumeroSemanas
				intensidadDocente[i].HorasSemanales += intensidad.HorasSemana
				intensidadDocente[i].Resoluciones = append(intensidadDocente[i].Resoluciones, models.ResolucionDocente{
					ProyectoCurricular: intensidad.ProyectoCurricular,
					Asignaturas:        intensidad.Asignaturas,
					FechaInicio:        intensidad.FechaInicio,
					FechaFin:           intensidad.FechaFin,
					HorasSemanales:     intensidad.HorasSemana,
					NumeroSemanas:      intensidad.NumeroSemanas,
					HorasSemestre:      intensidad.HorasSemestre,
					TipoVinculacion:    intensidad.TipoVinculacion,
					Categoria:          intensidad.Categoria,
					Valor:              intensidad.Valor,
					NivelAcademico:     intensidad.NivelAcademico,
					Facultad:           intensidad.Facultad,
					Resolucion:         intensidad.Resolucion,
				})
				break
			}
		}
...
// helper que ordena las intensidades horarias por año y periodo
...
// 	if response, err := getJsonTest(beego.AppConfig.String("administrativa_amazon_api_url")+"/informacion_proveedor/?query=NumDocumento:"+docDocente, &respuesta_proveedor); err != nil && response != 200 {
// 		outputError = map[string]interface{}{
// 			"Succes":  false,
// 			"Status":  404,
// 			"Message": "Error al obtener la informacion del proveedor",
...
// 	//fmt.Println("Url contrato: ", beego.AppConfig.String("administrativa_amazon_api_url")+"/contrato_general/?query=TipoContrato.Id__in:2|3|18,Contratista:"+strconv.Itoa(informacion_proveedor.Id)+"&limit=-1")
// 	if response, err := getJsonTest(beego.AppConfig.String("administrativa_amazon_api_url")+"/contrato_general/?query=TipoContrato.Id__in:2|3|18,Contratista:"+strconv.Itoa(informacion_proveedor.Id)+"&limit=-1", &respuesta_contrato); err != nil && response != 200 {
// 		outputError = map[string]interface{}{
// 			"Succes":  false,
// 			"Status":  404,
